...
function diffLists(before, after) {
	...
	const removed = [
		...new Set(before.filter((value) => !after.includes(value))),
	]
	return {
...
function getSelectors(versionPath) {
	const stats = JSON.parse(
		fs.readFileSync(join(versionPath, './stats/primer.json'))
	)
...
function getVariables(versionPath) {
	const variables = JSON.parse(
		fs.readFileSync(join(versionPath, './variables.json'))
	)
...
export function getPackageStats(packageName) {
	const stats = JSON.parse(
		fs.readFileSync(
			join(currentPath, './dist', `./stats/${packageName}.json`)
		)
	)
	return stats
}
...
function getDeprecations(versionPath) {
	const deprecations = JSON.parse(
		fs.readFileSync(join(versionPath, './deprecations.json'))
	)
	return deprecations
}
