...
async function analyzeVariables(fileName) {
	...
	for (const message of result.messages) {
		if (
			message.plugin === 'postcss-simple-vars' &&
			message.type === 'variable'
		) {
			if (
				!result.variables[`$${message.name}`].values.includes(
					message.value
				)
			) {
			...
			try {
				const c = `--temp-property: calc(${message.value})`.replace(
					'round(',
					'('
				)
			...
			return {
				...
				Declaration(node) {
					if (checkNode(node)) {
						...
						const fileName = node.source.input.file.replace(
							`${process.cwd()}/`,
							''
						)
						variables[node.prop] = {
