...
function getBundles(path) {
	...
	metaBundles = metaBundles.filter(
		(bundle) => !isSupportBundleName(bundle.name)
	)
	const bundles = {}
	for (const bundle of metaBundles) {
		const stats = JSON.parse(
			fs.readFileSync(join(path, `./${bundle.stats}`))
		)
		const entry = {
			name: bundle.name,
			path: bundle.css,
...
;(async () => {
	...
	const latestBundles = getBundles(
		join(__dirname, '../tmp/node_modules/@primer/css/')
	)
	...
	for (const name of Object.keys(currentBundles)) {
		const {
			stats: {
				selectors: { total: current_selectors_total },
				gzipSize: current_gzip_size,
				size: current_size,
			},
		} = currentBundles[name] || {
			stats: { selectors: { total: 0 }, gzipSize: 0, size: 0 },
		}

		const {
			stats: {
				selectors: { total: latest_selectors_total },
