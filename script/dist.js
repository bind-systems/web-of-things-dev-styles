	...
	try {
		...
		const tasks = files.map(async (from) => {
			...
			if (warnings.length) {
				...
				throw new Error(
					`Warnings while compiling ${from}.  See output above.`
				)
			}
			...
			await Promise.all([
				...
				writeFile(
					meta.stats,
					JSON.stringify(cssstats(result.css)),
					encoding
				),
				writeFile(
					meta.js,
					`export {cssstats: require('./stats/${name}.json')}`,
					encoding
				),
				result.map
					? writeFile(meta.map, result.map.toString(), encoding)
					: null,
			])
			bundles[name] = meta
		})
		...
		await copy(
			join(inDir, 'deprecations.json'),
			join(outDir, 'deprecations.json')
		)
	} catch (error) {
		console.error(error)
		process.exitCode = 1
